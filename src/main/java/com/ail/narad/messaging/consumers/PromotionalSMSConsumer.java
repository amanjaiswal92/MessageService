...
public class PromotionalSMSConsumer implements Consumer {
	...
	public PromotionalSMSConsumer(MessageSenderModel messageSenderModel) {
	...
	public void handleDelivery(String arg0, Envelope env,
		...
		try {
	        PromotionalSMSMessage messageBean = (PromotionalSMSMessage) PromotionalSMSMessage.deSerialize(message);
	        if(auditLogger != null) {
                addToLogService.addToAuditTable(new Audit_logs(messageBean.getRequestId(), AuditEvent.DEQUEUED_FROM_QUEUE, ""));
	        }
	        this.messageSenderModel.sendMessage(messageBean.getRequestId(),TemplateType.PROMOTIONAL_SMS, messageBean);
	        if(auditLogger != null) {
