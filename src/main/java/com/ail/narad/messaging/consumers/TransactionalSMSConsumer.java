...
public class TransactionalSMSConsumer implements Consumer {

	private MessageSenderModel messageSenderModel;

    @Autowired
    AddToLogService addToLogService;
	public TransactionalSMSConsumer(MessageSenderModel messageSenderModel) {
	...
	public void handleDelivery(String arg0, Envelope env,
		...
		try {
	        TransactionalSMSMessage messageBean = (TransactionalSMSMessage) TransactionalSMSMessage.deSerialize(message);
	        if(auditLogger != null) {
                addToLogService.addToAuditTable(new Audit_logs(messageBean.getRequestId(), AuditEvent.DEQUEUED_FROM_QUEUE, ""));
	        }
	        this.messageSenderModel.sendMessage(messageBean.getRequestId(),TemplateType.TRANSACTIONAL_SMS, messageBean);
