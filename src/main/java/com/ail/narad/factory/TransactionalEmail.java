...
import com.ail.narad.messaging.consumers.TransactionalEmailConsumer;
...
public class TransactionalEmail implements IMessageType {
	...
	public Boolean addToQueue(IMessageBean messageBean) throws Exception {
		try {
			RabbitMqUtils.enqueueMsgs(messageBean, TemplateType.TRANSACTIONAL_EMAIL.toString());
			return true;
		} catch (IOException e) {
			e.printStackTrace();
	...
	public IMessageBean getMessageBean(String type,Templates template, JSONObject dataJson, JSONObject metaJson) throws JSONException {
		...
		TransactionalEmailMessage message = new TransactionalEmailMessage();
	...
	public void consume(MessageSenderModel messageSenderModel) throws IOException {
		RabbitMqClient.consumeMsgs(new TransactionalEmailConsumer(messageSenderModel), TemplateType.TRANSACTIONAL_EMAIL.toString());
	}
