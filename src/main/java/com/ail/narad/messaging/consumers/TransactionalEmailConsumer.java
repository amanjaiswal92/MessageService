...
public class TransactionalEmailConsumer implements Consumer {

	private MessageSenderModel messageSenderModel;

    @Autowired
    AddToLogService addToLogService;
	public TransactionalEmailConsumer(MessageSenderModel messageSenderModel) {
	...
	public void handleDelivery(String arg0, Envelope env,BasicProperties arg2, byte[] body) throws IOException {
		...
		try {
	        ...
	        this.messageSenderModel.sendMessage(messageBean.getRequestId(),TemplateType.TRANSACTIONAL_EMAIL, messageBean);
	        if(auditLogger != null) {
