...
public class PromotionalEmailConsumer implements Consumer {
	...
	public PromotionalEmailConsumer(MessageSenderModel messageSenderModel) {
	...
	@Override
	public void handleDelivery(String arg0, Envelope env,	BasicProperties arg2, byte[] body) throws IOException {
		AuditLogger auditLogger = LogUtils.getAuditLogger();
		String message = new String(body, "UTF-8");
		System.out.println(" [x] Received '" + env.getRoutingKey() + "':'" + message + "'");
		try {
	        PromotionalEmailMessage messageBean = (PromotionalEmailMessage) PromotionalEmailMessage.deSerialize(message);
	        if(auditLogger != null) {
                addToLogService.addToAuditTable(new Audit_logs(messageBean.getRequestId(), AuditEvent.DEQUEUED_FROM_QUEUE, ""));
	        }
	        this.messageSenderModel.sendMessage(messageBean.getRequestId(),TemplateType.PROMOTIONAL_EMAIL, messageBean);
	        if(auditLogger != null) {
